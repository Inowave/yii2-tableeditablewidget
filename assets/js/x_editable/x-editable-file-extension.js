...
title: 'Enter file, caption and url #',
 ...
 https://github.com/vitalets/x-editable/issues/50
 **/
...
(function ($) {
    ...
    $.extend(File.prototype, {
		...
		activate: function() {
			...
			$(this.options.scope).editable('option', 'params', function(p){
				...
				d.append('image_upload', self.$input.filter('[name="image_upload"]').val());
				d.append('file_type', self.$input.filter('[name="file_type"]').val());
				return d
			})
    ...
    File.defaults = $.extend({}, $.fn.editabletypes.abstractinput.defaults, {
		/* file input with random name */ // '+ Math.random().toString().slice(2,11)+'
        tpl: '<div class="editable-file">' +
			...
			'<input type="hidden" class="image_upload" name="image_upload" value="0" />'+
			'<input type="hidden" class="file_type" name="file_type" value="" />'+
            '<button type="button" style="cursor: pointer; display: inline-block; margin-right: 5px;  " class="choose_file_button">Choose file</button>' +
            '</span></div>',
